    ...
    public static function assertSafeTestEnvironment(PDO $connection, string $databaseName): void
    {
        ...
        if ($currentDb === 'atlas' || $currentDb === 'sicrepr' || $currentDb === 'sicregero') {
            throw new RuntimeException(sprintf(
                'FATAL SECURITY GUARD: Test suite is refusing to execute against production database "%s". All integration tests MUST connect to an isolated test database (e.g. atlas_test).',
                $databaseName
            ));
        }
